...
| `.rh [表达式] [理由]` | 暗骰，结果私聊发给掷骰者，群内只公布表达式 |
| `.set [面数]` | 查看或设置默认骰 |
| `.ra <技能值> [理由]` | 技能检定，如 `.ra 60 侦查` |
| `.rab` / `.rap` | 带奖励骰或惩罚骰的检定，如 `.rab 60 侦查`、`.rab2 60`（两个奖励骰） |
| `.rah` / `.rha` | 暗检定，结果私聊发给发起者 |
| `.rahb` / `.rahp` / `.rhab` / `.rhap` | 带奖励骰或惩罚骰的暗检定 |
| `.rule [规则]` | 查看或设置检定规则（`coc7`、`simple`） |
| `.nn [昵称]` | 设置昵称，`.nn 清除` 恢复默认 |
| `.bot on` / `.bot off` | 启用或停用机器人，仅限群主与管理员 |
| `.plugin` / `.plugin off dice` | 查看或启停插件，仅限群主与管理员 |
| `.help` | 列出可用指令 |
| `.ping` | 确认机器人在线 |

每条指令都有中文别名，如 `.掷骰`、`.检定`、`.昵称`；前缀也可用全角句号 `。`。

重复执行的次数写在指令之后或行尾都可以，`.r 3#1d100`、`.r3#1d100` 与 `.r 1d100#3` 等价。
`N` 为一到三位的正整数，其余写法（含 `#0`）视为正文的一部分；两处同时写出时无法判断以
谁为准，按未命中处理。

暗骰与暗检定经主动消息投递，用户若在客户端关闭了机器人的主动消息则收不到，此时会在
群内追加一句提示。

### 掷骰表达式

表达式采用 [OneDice 标准](https://github.com/OlivOS-Team/onedice)的子集：

| 写法 | 含义 | 例 |
| --- | --- | --- |
| `AdB` | A 个 B 面骰，A 与 B 都可缺省 | `.r`、`.r 5d`、`.r 3d6` |
| `AdBkC` | 取最高的 C 个相加 | `.r 4d6k3` |
| `AdBqC` | 取最低的 C 个相加 | `.r 4d6q3` |
| `bN` / `pN` | 带 N 个奖励骰或惩罚骰的 d100，N 缺省为 1 | `.r b`、`.r p2` |
| `AdBbC` / `AdBpC` | A 个各带 C 个奖惩骰的 d100 | `.r 2db3` |
| 混合运算 | `+` `-` `*` `/`、乘方 `^` 与括号 | `.r d50*3+(2-p2)` |

奖惩骰把 d100 拆成十位骰与个位骰：追加若干个十位骰后，奖励骰取结果最小的一个，惩罚骰取
最大的一个，个位骰只掷一次。十位与个位都是 0 时结果为 100。奖惩骰因此固定为 D100，
`.r 2d20b1` 会报错，也不受 `.set` 所设默认骰的影响。

输出会展开被舍弃的十位骰，便于验算：

```
.r b3     →  D100B3=25[奖励骰:6 0 1]=5
.rab 60   →  D100B=63[奖励骰:1]=13/60，极难成功
.rab2 60  →  D100B2=63[奖励骰:1 8]=13/60，极难成功
```

骰池（`a`）、双重十字（`c`）、命运骰（`f`）、多元组与循环不在支持范围内。
...
- 需要在开放平台配置 **IP 白名单**，家用宽带的动态 IP 会导致请求被拒
...
检定规则是 `data/rules/*.json`，首次启动时自动写入内置的 `coc7` 与 `simple`，之后可直接编辑——
比如把极难成功从五分之一改成三分之一：

```json
{ "name": "极难成功", "description": "骰值不大于技能值的三分之一", "condition": "roll <= skill // 3" }
```

`condition` 中只能使用 `skill` 与 `roll` 两个变量以及算术、比较与布尔运算，函数调用一类会被拒绝。
等级按顺序取首个匹配，故大成功、大失败这类特例要排在普通成功之前。改完重启生效；写错会在启动时
直接报错并指明是哪份文件的哪一级。新增规则只需放一份新文件，文件名与其中的 `id` 保持一致，之后
在群里用 `.rule <id>` 切换。
