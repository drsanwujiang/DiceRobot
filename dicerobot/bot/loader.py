...
内置插件走显式清单，第三方插件走 entry points。两者都在启动期加载，任何一个失败
都直接抛出——静默跳过一个插件只会表现为某条指令"莫名其妙不响应"，难以排查。
...
def load_registry(*, include_entry_points: bool = True) -> Registry:
    """加载全部插件并装配注册表。
