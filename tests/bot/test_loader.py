...
class TestBuiltinPlugins:
    def test_loads_every_builtin(self) -> None:
        names = {plugin.name for plugin in load_registry(include_entry_points=False).plugins}

        assert names == {"dice", "check", "nickname", "system"}

    @pytest.mark.parametrize("alias", ["r", "set", "ra", "rule", "nn", "ping", "help", "bot", "plugin"])
    def test_every_expected_alias_resolves(self, alias: str) -> None:
        assert load_registry(include_entry_points=False).resolve(f".{alias}") is not None

    def test_system_plugin_sees_plugins_loaded_before_it(self) -> None:
        """系统插件最后构造，但捕获的是注册表对象，因此能看到全部插件。"""

        loaded = load_registry(include_entry_points=False)

        assert loaded.get("system") is not None
        assert len(loaded.plugins) == 4

    def test_system_plugin_cannot_be_disabled(self) -> None:
        system = load_registry(include_entry_points=False).get("system")
...
class TestFailures:
    def test_missing_module_is_reported(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with pytest.raises(ConfigurationError, match="导入失败"):
            load_registry(include_entry_points=False)

    def test_module_without_a_plugin_object_is_reported(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestEntryPoints:
    def test_third_party_plugin_is_registered(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        loaded = load_registry(include_entry_points=True)

        assert loaded.get("cards") is third_party

    def test_a_failing_entry_point_is_reported(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with pytest.raises(ConfigurationError, match="加载失败"):
            load_registry(include_entry_points=True)

    def test_a_non_plugin_entry_point_is_reported(self, monkeypatch: pytest.MonkeyPatch) -> None:
