...
提交：一个提交只做一件事，且应能独立通过测试。主题直述改动，不加类型前缀；正文写清为什么这样改
——问题的表现、原因与取舍，而非复述 diff。仅改动文字的提交注明「无行为变更」。
...
2. `bot/pipeline.py::submit` 去重（`bot/dedup.py`，TTL 10 分钟）后入队；队列满时**丢弃并告警**而非
   反压。去重在入队之前，故因队列满而丢弃的事件，其重推同样被拦下：重推的间隔足以让同一条指令
   执行两次。webhook 处理函数不得阻塞。
...
5. `bot/registry.py::resolve` 解析指令：要求 `.` 或 `。` 前缀，**最长别名优先**（故 `.rab` 命中奖励
   ...
   两处都写出时无法判断以谁为准，按未命中处理。未命中即丢弃——群可能开启全量消息推送，快速路径
   开销要低。
...
不做；`kh`/`kl` 同样不做——它们只在骰组与多元组上有意义，且 `6dkh` 等价于 `6dk1`，没有表达力损失。

**取低是 `q` 不是 `kl`**：`kl` 在 OneDice 中另有含义（取结果中的最小值），沿用会与生态长期不一致。
...
奖惩骰只改变 d100 的掷法，不改变判定，故 `trpg/check.py` 与规则文件都不感知它。检定的奖惩骰由
指令别名指定（`.rab` / `.rap` / `.rahb` 一类，与 OlivaDice 的 `.ra(b/p)` 一致）而非写在参数里：检定
的参数是技能值而非表达式，修饰符写在参数里会与检定理由的位置冲突。

个数则只能写在参数里：别名匹配是最长前缀，`.rab2 60` 剥掉 `rab` 之后是 `2 60`，与 `.rab 2 60` 同形，
两者都是两个奖励骰。数字之后紧跟另一个数字时才是个数，且取值须在 1 到 `MAX_MODIFIER_DICE` 之间，
否则按技能值处理——`.rab 60 2` 的 60 是技能值。
